...
public class AtomicOperationTransitionDestroyScope implements AtomicOperation {
  ...
  private static Logger log = LoggerFactory.getLogger(AtomicOperationTransitionDestroyScope.class.getName());
  ...
  public void execute(InterpretableExecution execution) {
    ...
    if (activity.isScope()) {
      ...
      if (execution.isConcurrent() && !execution.isScope()) {
        ...
        log.debug("moving concurrent "+execution+" one scope up under "+parentScopeInstance);
        ...
        if (concurrentRootExecutions.size()==1) {
          ...
          if (lastConcurrent.isScope()) {
            ...
            log.debug("merging last concurrent "+lastConcurrent+" into concurrent root "+concurrentRoot);
        ...
        log.debug("scoped concurrent "+execution+" becomes concurrent and remains under "+execution.getParent());
        ...
        log.debug("destroy scope: scoped "+execution+" continues as parent scope "+propagatingExecution);
        execution.destroy();
        execution.remove();
      }
